...
export default function FindColleges({ user, setActiveTab }: FindCollegesProps) {
  ...
  // Helper to determine fit color (Premium Black minimalist approach)
  const getFitColor = (score: number) => {
    return { bg: 'bg-white', text: 'text-white', bgLight: 'bg-white/10' };
  };
  ...
  return (
    <div className="w-full max-w-5xl space-y-8 pb-12" id="find-colleges-tab">
      ...
      {/* Header Panel */}
      <div className="bg-[#111111] border border-white/5 rounded-2xl p-8 text-white relative overflow-hidden" id="find-header">
      ...
      {/* Filter Dashboard */}
      <div className="bg-[#111111] border border-white/5 rounded-2xl p-6 shadow-2xl" id="filter-dashboard">
        <h2 className="text-xs font-bold text-white/40 uppercase tracking-widest mb-4">Refine College Filters</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
          
          {/* Target Country */}
          <div className="space-y-1">
            <label className="text-[10px] font-bold text-white/50 uppercase tracking-wider">Country</label>
            <select
              value={selectedCountry}
              onChange={(e) => setSelectedCountry(e.target.value)}
              className="w-full px-3 py-2 text-xs font-medium bg-black border border-white/10 text-white rounded-lg focus:outline-none focus:border-white focus:ring-1 focus:ring-white/20 transition-all"
            >
          ...
          {/* Academic Major */}
          <div className="space-y-1">
            <label className="text-[10px] font-bold text-white/50 uppercase tracking-wider">Preferred Major</label>
            <input
              type="text"
          ...
          {/* Target Degree */}
          <div className="space-y-1">
            <label className="text-[10px] font-bold text-white/50 uppercase tracking-wider">Degree Level</label>
            <select
          ...
          {/* Budget Range limits */}
          <div className="space-y-1">
            <label className="text-[10px] font-bold text-white/50 uppercase tracking-wider">Budget Limit (Yr)</label>
            <input
              type="text"
        ...
        <div className="mt-6 flex justify-end">
          <button
            ...
            disabled={loading}
            className="px-5 py-2.5 bg-white hover:bg-white/90 disabled:bg-white/20 disabled:text-white/40 text-black text-xs font-bold rounded-lg shadow-lg transition-all flex items-center space-x-2 cursor-pointer hover:scale-[1.02] active:scale-[0.98]"
          >
            {loading ? (
      ...
      {/* Error Panel */}
      {error && (
        <div className="bg-red-950/20 border border-red-500/15 rounded-xl p-4 flex items-start space-x-3 text-xs text-red-400">
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="font-bold text-white text-base">Recommended Institutions</h2>
          <span className="text-xs text-white/40 font-medium">Found {colleges.length} matches</span>
        </div>

        {loading ? (
          <div className="flex flex-col items-center justify-center py-20 space-y-4 bg-[#111111] rounded-2xl border border-white/5 shadow-2xl">
            <Loader2 className="w-10 h-10 text-white animate-spin" />
            <div className="text-center space-y-1">
              <p className="text-xs font-bold text-white">Querying Global Institutional Records...</p>
              <p className="text-[10px] text-white/40">Gemini is running alignment matrices against tuition, requirements, and profiles.</p>
            </div>
          </div>
        ) : colleges.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {colleges.map((college, idx) => {
              ...
              return (
                <div 
                  key={idx}
                  className={`bg-[#111111] rounded-2xl p-6 border ${isShortlisted ? 'border-white ring-2 ring-white/5 shadow-2xl' : 'border-white/5'} hover:border-white/10 transition-all flex flex-col justify-between`}
                >
                  <div className="space-y-4">
                    ...
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <span className="shrink-0 bg-white/5 text-white/90 text-[10px] font-bold px-2 py-1 rounded border border-white/5">
                    ...
                    <div className="grid grid-cols-2 gap-3 bg-white/5 p-3 rounded-xl border border-white/5">
                      <div>
                        <span className="block text-[9px] text-white/40 font-bold uppercase tracking-wider">Avg Tuition</span>
                        <span className="text-xs font-extrabold text-white">{college.avgTuition}</span>
                      </div>
                      <div>
                        <span className="block text-[9px] text-white/40 font-bold uppercase tracking-wider">Acceptance</span>
                        <span className="text-xs font-extrabold text-white">{college.acceptanceRate}</span>
                      </div>
                    </div>
                    ...
                    <div className="space-y-1.5">
                      <div className="flex justify-between items-center text-[10px] font-bold">
                        <span className="text-white/40 uppercase tracking-widest">Student Fit Match</span>
                        <span className="text-white">{college.fitScore}%</span>
                      </div>
          ...
          <div className="bg-[#111111] border border-dashed border-white/10 rounded-2xl py-14 px-8 text-center max-w-lg mx-auto space-y-4">
            <div className="w-12 h-12 bg-white/5 text-white/60 rounded-full flex items-center justify-center mx-auto border border-white/5">
              <Compass className="w-6 h-6 stroke-[1.5]" />
            </div>
            <div className="space-y-1">
              <h3 className="font-bold text-white text-xs">Begin Your Search</h3>
      ...
      {shortlist.length >= 2 && (
        <div className="fixed bottom-14 right-8 z-50 animate-bounce">
          <button
            ...
            className="px-5 py-3 bg-white text-black font-extrabold text-xs rounded-full shadow-2xl hover:bg-white/90 transition-all flex items-center space-x-2 cursor-pointer border border-white/10"
          >
